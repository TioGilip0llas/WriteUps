...
>Redis es un protocolo basado en texto. Envía el comando en un socket y los valores devueltos serán legibles. Puede ejecutarse con SSL/TLS (aunque esto es bastante inusual).
En una instancia normal de Redis, puedes conectarte usando nc o también puedes usar redis-cli:
	`nc -vn 10.10.10.10 6379`
	`redis-cli -h 10.10.10.10 # sudo apt-get install redis-tools`

Comenzamos a probar concienzudamente y también el `cli`. Dando los primeros pasos, logramos hacer una key de prueba:
```java
...
Tomando en cuenta la opción mediante `nc`, también podemos enumerar información.
...
Listaremos las bases de datos y número de claves con `INFO keyspace`. No parece de mucha utilidad, pero será utilizado para después.
```java
INFO keyspace
$44
# Keyspace
...
Podemos notar que está nuestra llave de prueba:
```java
get  my_key
$6
Tumama
get flag
$32
03e1d2b376t3L4c0M15t3eNT3r4953eb
```
